...
impl fmt::Display for AssemblyError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        use AssemblyError::{
            CallInKernel, CallerOutOKernel, CircularModuleDependency, DivisionByZero,
            DuplicateProcName, ExportedProcInProgram, ImportedProcModuleNotFound,
            ImportedProcNotFoundInModule, InvalidCacheLock, Io, KernelProcNotFound, LibraryError,
            LocalProcNotFound, ParamOutOfBounds, ParsingError, ProcedureNameError, SysCallInKernel,
        };
        match self {
...
impl fmt::Display for ProcedureNameError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        use ProcedureNameError::{
            EmptyProcedureName, InvalidFirstLetter, InvalidProcedureName, ProcedureNameTooLong,
        };
        match self {
...
impl fmt::Display for SerializationError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        use SerializationError::{
            EndOfReader, InvalidBoolValue, InvalidFieldElement, InvalidModulePath,
            InvalidNamespace, InvalidNumOfPushValues, InvalidNumber, InvalidOpCode,
            InvalidPathNoDelimiter, InvalidProcedureName, InvalidUtf8, LengthTooLong,
            UnexpectedEndOfStream,
        };
        match self {
...
impl fmt::Display for LibraryError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        use LibraryError::{
            DuplicateModulePath, DuplicateNamespace, EmptyProcedureName, LibraryNameWithDelimiter,
            ModuleNotFound, ModulePathEndsWithDelimiter, ModulePathStartsWithDelimiter,
            NamespaceViolation, ProcedureNameWithDelimiter,
        };
        match self {
