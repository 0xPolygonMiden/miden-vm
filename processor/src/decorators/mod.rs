    ...
    A: AdviceProvider,
{
    ...
    /// Injects a node of the Merkle tree specified by the values on the stack at the top of the
    ...
    fn inject_merkle_node(&mut self) -> Result<(), ExecutionError> {
        ...
        self.advice_provider.write_stack(AdviceSource::Value(node[3]))?;
        self.advice_provider.write_stack(AdviceSource::Value(node[2]))?;
        self.advice_provider.write_stack(AdviceSource::Value(node[1]))?;
        self.advice_provider.write_stack(AdviceSource::Value(node[0]))?;
    ...
    fn inject_div_result_u64(&mut self) -> Result<(), ExecutionError> {
        ...
        self.advice_provider.write_stack(AdviceSource::Value(r_hi))?;
        self.advice_provider.write_stack(AdviceSource::Value(r_lo))?;
        self.advice_provider.write_stack(AdviceSource::Value(q_hi))?;
        self.advice_provider.write_stack(AdviceSource::Value(q_lo))?;
    ...
    fn inject_map_value(&mut self) -> Result<(), ExecutionError> {
        ...
        self.advice_provider.write_stack(AdviceSource::Map { key: top_word })?;
    ...
    fn inject_ext2_inv_result(&mut self) -> Result<(), ExecutionError> {
        ...
        self.advice_provider.write_stack(AdviceSource::Value(coeffs[1]))?;
        self.advice_provider.write_stack(AdviceSource::Value(coeffs[0]))?;

        Ok(())
    }

    /// Given evaluations of a polynomial over some specified domain, this routine interpolates the
    /// evaluations into a polynomial in coefficient form, and pusshes the results onto the advice
    /// stack.
    ///
    ...
    fn inject_ext2_intt_result(&mut self) -> Result<(), ExecutionError> {
        ...
        for i in Ext2Element::as_base_elements(&poly[..out_poly_len]).iter().rev().copied() {
            self.advice_provider.write_stack(AdviceSource::Value(i))?;
        }
