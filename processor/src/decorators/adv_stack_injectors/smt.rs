use super::{
    AdviceProvider, AdviceSource, ExecutionError, Felt, Process, StarkField, Word, ONE, WORD_SIZE,
    ZERO,
};
use vm_core::{
    crypto::{
    ...
    A: AdviceProvider,
{
    ...
    pub(in crate::decorators) fn push_smtget_inputs(&mut self) -> Result<(), ExecutionError> {
    ...
    /// # Panics
    /// Will panic as unimplemented if the target depth is `64`.
    pub(in crate::decorators) fn push_smtpeek_result(&mut self) -> Result<(), ExecutionError> {
        // fetch the arguments from the operand stack
        let key = self.stack.get_word(0);
        let root = self.stack.get_word(1);
    ...
    /// # Panics
    /// Will panic as unimplemented if the target depth is `64`.
    pub(in crate::decorators) fn push_smtset_inputs(&mut self) -> Result<(), ExecutionError> {
