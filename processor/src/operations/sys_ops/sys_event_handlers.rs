...
pub fn copy_merkle_node_to_adv_stack(
    ...
    let node = advice_provider.get_tree_node(root.into(), &depth, &index).map_err(err_map)?;
    ...
    if node == *empty_leaf {
        ...
        for (key_in_leaf, value_in_leaf) in leaf_preimage {
            if key == key_in_leaf {
                ...
                advice_provider.push_stack(AdviceSource::Word(value_in_leaf)).map_err(err_map)?;
...
fn get_mem_addr_range(
    ...
    if start_addr > u32::MAX as u64 {
        return Err(MemoryError::address_out_of_bounds(start_addr, &ErrorContext::default()));
    }
    if end_addr > u32::MAX as u64 {
        return Err(MemoryError::address_out_of_bounds(end_addr, &ErrorContext::default()));
    }
