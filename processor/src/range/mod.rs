...
/// starts with value 0 and end with value 65535. The values must be in increasing order but can be
/// up to 8 values apart, and duplicates are allowed.
///
...
/// - Column `v` contains the value being range-checked where `v` must be a 16-bit value.
...
/// (1, 1, v) and (1, 0, v). The first row specifies that there was 4 lookups and the second
...
impl RangeChecker {
    ...
    pub fn into_trace_with_table(
        ...
        for (&value, &num_lookups) in self.lookups.iter() {
            // when the delta between two values is greater than 8, insert "bridge" rows
            for value in (prev_value..value).step_by(8).skip(1) {
    ...
    /// Returns the number of 16-bit table rows needed to support all 16-bit lookups.
    pub fn get_number_range_checker_rows(&self) -> usize {
        ...
        let mut prev_value = 0u16;
        for (&value, &num_lookups) in self.lookups.iter() {
            // determine how many 16-bit lookup rows we need for this value
            ...
            // since rows in the range checker table can be at most 8 values apart.
            let delta = value - prev_value;
            let (delta_q, delta_r) = div_rem(delta as usize, 8);

            if delta_q != 0 {
                let num_bridge_rows = if delta_r == 0 { delta_q - 1 } else { delta_q };
                num_rows += num_bridge_rows;
            }
