...
fn dyn_block() {
    ...
    let push_node = MastNode::new_basic_block(
        vec![Operation::Push(Felt::from(FOO_ROOT_NODE_ADDR as u32))],
        None,
    )
    .unwrap();
    ...
    check_op_decoding(
        &trace,
        6,
        push_basic_block_addr,
        Operation::Push(Felt::from(FOO_ROOT_NODE_ADDR as u32)),
        1,
        0,
        1,
    );
    ...
    assert_eq!(
        [ZERO, ZERO, foo_hash[0], foo_hash[1], foo_hash[2], foo_hash[3], ZERO, ZERO],
        get_hasher_state(&trace, 10)
    );
