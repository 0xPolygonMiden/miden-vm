...
pub trait AdviceProvider: Sized {
    ...
    /// Drains the top `len` elements from the advice stack and returns them as a vector.
    ///
    /// Note: The resulting vector will contain elements in the order in which they were pushed
    /// on to the stack. For example, a `[h, g, f, e, d, c, b, a, ...]` stack (i.e., `h` is at the
    /// top of the stack) will yield `[e, f, g, h]` if `len = 4`.
    ///
    /// # Errors
    /// Returns an error if the advice stack does not contain `len` elements.
