...
pub(crate) fn insert_mem_values_into_adv_map<S: ProcessState, A: AdviceProvider>(
    ...
    for addr in start_addr..end_addr {
        let mem_value = process.chiplets().get_mem_value(ctx, addr).unwrap_or(EMPTY_WORD);
...
fn get_mem_addr_range<S: ProcessState>(
    ...
    let start_addr = process.stack().get(start_idx).as_int();
    let end_addr = process.stack().get(end_idx).as_int();
