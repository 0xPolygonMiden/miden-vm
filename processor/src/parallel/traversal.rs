...
impl ExecutionTraversal {
    ...
    pub fn advance(&mut self) -> Option<MastNodeId> {
        ...
        // Get the current node to determine how to proceed
        let current_node = self
            .mast_forest
            .get_node_by_id(current_node_id)
            .expect("Current node should exist in the forest");
        ...
        match current_node {
            ...
            | MastNode::Dyn(_)
            | MastNode::External(_) => {
                // When advance() is called, the current node is considered fully executed
                // Pop it and continue with parent's execution order
                self.call_stack.pop();
