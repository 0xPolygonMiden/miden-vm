...
pub struct ExternalNodeReplay {
    pub external_node_resolutions: VecDeque<MastNodeId>,
}
...
impl ExternalNodeReplay {
    ...
    /// Records a resolution of an external node to a MastNodeId
    pub fn record_resolution(&mut self, node_id: MastNodeId) {
        self.external_node_resolutions.push_back(node_id);
    }

    /// Replays the next recorded external node resolution
    pub fn replay_resolution(&mut self) -> MastNodeId {
...
impl AdviceReplay {
    ...
    pub fn record_map_get(&mut self, key: RpoDigest, values: Option<&[Felt]>) {
        match values {
            Some(values) => {
    ...
    pub fn replay_stack_push(&mut self, source: AdviceSource) {
        ...
        debug_assert_eq!(
            ...
            "AdviceSource mismatch: expected {:?}, got {:?}",
    ...
    /// the same key.
    pub fn replay_map_get(&mut self, key: &RpoDigest) -> Option<Vec<Felt>> {
        let (recorded_key, was_found) =
            self.map_gets.pop_front().expect("No map get operations recorded");
        debug_assert_eq!(
            recorded_key, *key,
            "Map key mismatch: expected {:?}, got {:?}",
            key, recorded_key
        );
    ...
    pub fn replay_map_insert(&mut self, key: Word, values: Vec<Felt>) {
        ...
        debug_assert_eq!(
            recorded_key, key,
            "Map insert key mismatch: expected {:?}, got {:?}",
            key, recorded_key
        );
        debug_assert_eq!(
            recorded_values, values,
            "Map insert values mismatch: expected {:?}, got {:?}",
    ...
    pub fn replay_tree_node(&mut self, root: Word, depth: &Felt, index: &Felt) -> Word {
        ...
        debug_assert_eq!(
            recorded_root, root,
            "Tree node root mismatch: expected {:?}, got {:?}",
            root, recorded_root
        );
        debug_assert_eq!(
            recorded_depth, *depth,
            "Tree node depth mismatch: expected {:?}, got {:?}",
            depth, recorded_depth
        );
        debug_assert_eq!(
            recorded_index, *index,
            "Tree node index mismatch: expected {:?}, got {:?}",
    ...
    pub fn replay_merkle_path(&mut self, root: Word, depth: Felt, index: Felt) -> MerklePath {
        ...
        debug_assert_eq!(
            recorded_root, root,
            "Merkle path root mismatch: expected {:?}, got {:?}",
            root, recorded_root
        );
        debug_assert_eq!(
            recorded_depth, depth,
    ...
    pub fn replay_leaf_depth(&mut self, root: Word, tree_depth: &Felt, index: &Felt) -> u8 {
        ...
        debug_assert_eq!(
            recorded_root, root,
            "Leaf depth root mismatch: expected {:?}, got {:?}",
            root, recorded_root
        );
        debug_assert_eq!(
            recorded_tree_depth, *tree_depth,
            "Leaf depth tree_depth mismatch: expected {:?}, got {:?}",
            tree_depth, recorded_tree_depth
        );
        debug_assert_eq!(
            recorded_index, *index,
            "Leaf depth index mismatch: expected {:?}, got {:?}",
            index, recorded_index
        );
        leaf_depth
    }
    ...
    pub fn replay_node_update(
        ...
        debug_assert_eq!(
            recorded_root, root,
            "Node update root mismatch: expected {:?}, got {:?}",
            root, recorded_root
        );
        debug_assert_eq!(
            recorded_depth, *depth,
            "Node update depth mismatch: expected {:?}, got {:?}",
            depth, recorded_depth
        );
        debug_assert_eq!(
            recorded_index, *index,
            "Node update index mismatch: expected {:?}, got {:?}",
            index, recorded_index
        );
        debug_assert_eq!(
            recorded_value, value,
            "Node update value mismatch: expected {:?}, got {:?}",
    ...
    pub fn replay_root_merge(&mut self, lhs: Word, rhs: Word) -> Word {
        ...
        debug_assert_eq!(
            recorded_lhs, lhs,
            "Root merge lhs mismatch: expected {:?}, got {:?}",
            lhs, recorded_lhs
        );
        debug_assert_eq!(
            recorded_rhs, rhs,
            "Root merge rhs mismatch: expected {:?}, got {:?}",
            rhs, recorded_rhs
        );
        result
    }
