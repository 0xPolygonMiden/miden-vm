...
use super::{MainTraceFragmentGenerator, TraceRowType};
...
impl MainTraceFragmentGenerator {
    ...
    pub fn add_control_flow_trace_row(
        ...
        // Increment the number of rows built
        self.num_rows_built += 1;
    }

    /// Populates the system trace columns with checkpoint data
    fn populate_system_trace_columns(&mut self, row_idx: usize) {
        self.fragment.columns[CLK_COL_IDX][row_idx] = Felt::from(self.checkpoint.system.clk); // clk
        self.fragment.columns[FMP_COL_IDX][row_idx] = self.checkpoint.system.fmp; // fmp
        self.fragment.columns[CTX_COL_IDX][row_idx] = Felt::from(self.checkpoint.system.ctx); // ctx
        self.fragment.columns[IN_SYSCALL_COL_IDX][row_idx] =
            if self.checkpoint.system.in_syscall { ONE } else { ZERO }; // in_syscall flag
        self.fragment.columns[FN_HASH_OFFSET][row_idx] = self.checkpoint.system.fn_hash[0]; // fn_hash[0]
        self.fragment.columns[FN_HASH_OFFSET + 1][row_idx] = self.checkpoint.system.fn_hash[1]; // fn_hash[1]
        self.fragment.columns[FN_HASH_OFFSET + 2][row_idx] = self.checkpoint.system.fn_hash[2]; // fn_hash[2]
        self.fragment.columns[FN_HASH_OFFSET + 3][row_idx] = self.checkpoint.system.fn_hash[3]; // fn_hash[3]
    }
    ...
    fn populate_decoder_trace_columns(
        ...
        match trace_type {
            ...
            TraceRowType::End => {
                ...
                // Second half remains ZERO for END operation
                self.fragment.columns[DECODER_TRACE_OFFSET + HASHER_STATE_OFFSET + 4][row_idx] =
                    ZERO; // hasher[4]
                self.fragment.columns[DECODER_TRACE_OFFSET + HASHER_STATE_OFFSET + 5][row_idx] =
                    ZERO; // hasher[5]
