...
impl AuxTraceBuilder {
    ...
    pub fn build_aux_columns<E: FieldElement<BaseField = Felt>>(
        ...
        let bus_col_builder = BusColumnBuilder::new("chiplets bus");
...
/// column.
pub struct ChipletsVTableColBuilder<E> where E: FieldElement<BaseField = Felt> {
    kernel: Kernel,
    // Not implemented yet
    #[cfg(any(test, feature = "bus-debugger"))]
    bus_debugger: core::cell::RefCell<BusDebugger<E>>,
    #[cfg(not(any(test, feature = "bus-debugger")))]
    bus_debugger: PhantomData<E>,
}

impl<E> ChipletsVTableColBuilder<E> where E: FieldElement<BaseField = Felt> {
    fn new(kernel: Kernel) -> Self {
        Self { kernel,
        #[cfg(any(test, feature = "bus-debugger"))]
        bus_debugger: core::cell::RefCell::new(BusDebugger::new("chiplets bus")),
        #[cfg(not(any(test, feature = "bus-debugger")))]
        bus_debugger: PhantomData,
        }
    }
}

impl<E: FieldElement<BaseField = Felt>> AuxColumnBuilder<E> for ChipletsVTableColBuilder<E> {
    fn init_requests(
        &self,
...
impl<E: FieldElement<BaseField = Felt>> BusColumnBuilder<E> {
    pub fn new(_bus_name: &'static str) -> Self {
        Self {
            #[cfg(any(test, feature = "bus-debugger"))]
            bus_debugger: core::cell::RefCell::new(BusDebugger::new(_bus_name)),
            #[cfg(not(any(test, feature = "bus-debugger")))]
            bus_debugger: PhantomData,
        }
    }
...
impl<E: FieldElement<BaseField = Felt>> AuxColumnBuilder<E> for BusColumnBuilder<E> {
        ...
        E: FieldElement<BaseField = Felt>,
    {
        ...
        match op_code {
            OPCODE_JOIN | OPCODE_SPLIT | OPCODE_LOOP | OPCODE_CALL => self
                .build_control_block_request(
                    main_trace,
            ...
            OPCODE_DYN | OPCODE_DYNCALL => {
                self.build_dyn_block_request(main_trace, op_code_felt, alphas, row)
            },
            OPCODE_SYSCALL => self.build_syscall_block_request(main_trace, op_code_felt, alphas, row),
            OPCODE_SPAN => self.build_span_block_request(main_trace, alphas, row),
            OPCODE_RESPAN => self.build_respan_block_request(main_trace, alphas, row),
            OPCODE_END => self.build_end_block_request(main_trace, alphas, row),
            OPCODE_U32AND => self.build_bitwise_request(main_trace, ZERO, alphas, row),
            OPCODE_U32XOR => self.build_bitwise_request(main_trace, ONE, alphas, row),
            OPCODE_MLOADW => {
                self.build_mem_mloadw_mstorew_request(main_trace, MEMORY_READ_WORD_LABEL, alphas, row)
            },
            OPCODE_MSTOREW => {
                self.build_mem_mloadw_mstorew_request(main_trace, MEMORY_WRITE_WORD_LABEL, alphas, row)
            },
            OPCODE_MLOAD => {
                self.build_mem_mload_mstore_request(main_trace, MEMORY_READ_ELEMENT_LABEL, alphas, row)
            },
            OPCODE_MSTORE => {
                self.build_mem_mload_mstore_request(main_trace, MEMORY_WRITE_ELEMENT_LABEL, alphas, row)
            },
            OPCODE_MSTREAM => self.build_mstream_request(main_trace, alphas, row),
            OPCODE_RCOMBBASE => self.build_rcomb_base_request(main_trace, alphas, row),
            OPCODE_HPERM => self.build_hperm_request(main_trace, alphas, row),
            OPCODE_MPVERIFY => self.build_mpverify_request(main_trace, alphas, row),
            OPCODE_MRUPDATE => self.build_mrupdate_request(main_trace, alphas, row),
            OPCODE_PIPE => self.build_pipe_request(main_trace, alphas, row),
            _ => E::ONE,
        }
    }
        ...
        E: FieldElement<BaseField = Felt>,
    {
        if main_trace.is_hash_row(row) {
            self.build_hasher_chiplet_responses(main_trace, row, alphas)
        } else if main_trace.is_bitwise_row(row) {
            self.build_bitwise_chiplet_responses(main_trace, row, alphas)
        } else if main_trace.is_memory_row(row) {
            self.build_memory_chiplet_responses(main_trace, row, alphas)
        } else if main_trace.is_kernel_row(row) {
            self.build_kernel_chiplet_responses(main_trace, row, alphas)
        } else {
            E::ONE
        }
    }
...
impl<E: FieldElement<BaseField = Felt>> BusColumnBuilder<E> {
    ...
    fn build_control_block_request(
        &self,
        main_trace: &MainTrace,
        ...
        #[cfg(any(test, feature = "bus-debugger"))]
        self.bus_debugger.borrow_mut().add_request(Box::new(message), alphas);

        value
    }

    /// Builds requests made on a `DYN` or `DYNCALL` operation.
    fn build_dyn_block_request(
        &self,
        main_trace: &MainTrace,
        op_code_felt: Felt,
        alphas: &[E],
        ...
        #[cfg(any(test, feature = "bus-debugger"))]
        {
            ...
            self.bus_debugger.borrow_mut().add_request(Box::new(memory_req), alphas);
        }
    ...
    fn build_syscall_block_request(
        &self,
        main_trace: &MainTrace,
        op_code_felt: Felt,
        alphas: &[E],
        ...
        #[cfg(any(test, feature = "bus-debugger"))]
        {
            self.bus_debugger.borrow_mut().add_request(Box::new(control_block_req), alphas);
            self.bus_debugger.borrow_mut().add_request(Box::new(kernel_rom_req), alphas);
        }
    ...
    fn build_span_block_request(&self, main_trace: &MainTrace, alphas: &[E], row: RowIndex) -> E {
        ...
        #[cfg(any(test, feature = "bus-debugger"))]
        self.bus_debugger.borrow_mut().add_request(Box::new(span_block_message), alphas);

        value
    }

    /// Builds requests made to the hasher chiplet at the start of a respan block.
    fn build_respan_block_request(&self, main_trace: &MainTrace, alphas: &[E], row: RowIndex) -> E {
        ...
        self.bus_debugger
            .borrow_mut()
            .add_request(Box::new(respan_block_message), alphas);

        value
    }

    /// Builds requests made to the hasher chiplet at the end of a block.
    fn build_end_block_request(&self, main_trace: &MainTrace, alphas: &[E], row: RowIndex) -> E {
        ...
        #[cfg(any(test, feature = "bus-debugger"))]
        self.bus_debugger.borrow_mut().add_request(Box::new(end_block_message), alphas);
    ...
    fn build_bitwise_request(
        ...
        self.bus_debugger
            .borrow_mut()
            .add_request(Box::new(bitwise_request_message), alphas);

        value
    }

    /// Builds `MSTREAM` requests made to the memory chiplet.
    fn build_mstream_request(&self, main_trace: &MainTrace, alphas: &[E], row: RowIndex) -> E {
        ...
        #[cfg(any(test, feature = "bus-debugger"))]
        {
            self.bus_debugger.borrow_mut().add_request(Box::new(mem_req_1), alphas);
            self.bus_debugger.borrow_mut().add_request(Box::new(mem_req_2), alphas);
        }
    ...
    fn build_pipe_request(&self, main_trace: &MainTrace, alphas: &[E], row: RowIndex) -> E {
    ...
    fn build_rcomb_base_request(&self, main_trace: &MainTrace, alphas: &[E], row: RowIndex) -> E {
    ...
    fn build_hperm_request(&self, main_trace: &MainTrace, alphas: &[E], row: RowIndex) -> E {
        ...
        #[cfg(any(test, feature = "bus-debugger"))]
        {
            self.bus_debugger.borrow_mut().add_request(Box::new(input_req), alphas);
            self.bus_debugger.borrow_mut().add_request(Box::new(output_req), alphas);
        }
    ...
    fn build_mpverify_request(&self, main_trace: &MainTrace, alphas: &[E], row: RowIndex) -> E {
        let helper_0 = main_trace.helper_register(0, row);
        ...
        #[cfg(any(test, feature = "bus-debugger"))]
        {
            self.bus_debugger.borrow_mut().add_request(Box::new(input), alphas);
            self.bus_debugger.borrow_mut().add_request(Box::new(output), alphas);
        }
    ...
    fn build_mrupdate_request(&self, main_trace: &MainTrace, alphas: &[E], row: RowIndex) -> E {
        let helper_0 = main_trace.helper_register(0, row);
        ...
        #[cfg(any(test, feature = "bus-debugger"))]
        {
            self.bus_debugger.borrow_mut().add_request(Box::new(input_old), alphas);
            self.bus_debugger.borrow_mut().add_request(Box::new(output_old), alphas);
            self.bus_debugger.borrow_mut().add_request(Box::new(input_new), alphas);
            self.bus_debugger.borrow_mut().add_request(Box::new(output_new), alphas);
        }
    ...
    fn build_mem_mloadw_mstorew_request(
        ...
        self.compute_mem_request_word(main_trace, op_label, alphas, row, addr, word)
    }
    ...
    fn build_mem_mload_mstore_request(
        ...
        self.compute_mem_request_element(main_trace, op_label, alphas, row, addr, element)
    }

    /// Computes a memory request for a read or write of a single element.
    fn compute_mem_request_element(
        &self,
        main_trace: &MainTrace,
        op_label: u8,
        alphas: &[E],
        ...
        debug_assert!(
            op_label == MEMORY_READ_ELEMENT_LABEL || op_label == MEMORY_WRITE_ELEMENT_LABEL
        );
    ...
    fn build_hasher_chiplet_responses(
        &self,
        main_trace: &MainTrace,
        row: RowIndex,
        alphas: &[E],
        ...
        if row.as_usize() % HASH_CYCLE_LEN == 0 {
            ...
            if selector1 == ONE && selector2 == ZERO && selector3 == ZERO {
                ...
                #[cfg(any(test, feature = "bus-debugger"))]
                self.bus_debugger.borrow_mut().add_response(Box::new(hasher_message), alphas);
            }
            ...
            if selector1 == ONE && !(selector2 == ZERO && selector3 == ZERO) {
                ...
                if bit == 0 {
                    let hasher_message = HasherMessage {
                        ...
                        node_index,
                        hasher_state: [
                            ZERO, ZERO, ZERO, ZERO, state[4], state[5], state[6], state[7], ZERO,
                            ZERO, ZERO, ZERO,
                        ],
                        source: "hasher",
                    };
    ...
    fn build_bitwise_chiplet_responses(
        &self,
        main_trace: &MainTrace,
        row: RowIndex,
        alphas: &[E],
        ...
        if row.as_usize() % BITWISE_OP_CYCLE_LEN == BITWISE_OP_CYCLE_LEN - 1 {
            ...
            #[cfg(any(test, feature = "bus-debugger"))]
            self.bus_debugger.borrow_mut().add_response(Box::new(bitwise_message), alphas);
    ...
    fn build_memory_chiplet_responses(
        &self,
        main_trace: &MainTrace,
        row: RowIndex,
        alphas: &[E],
        ...
        #[cfg(any(test, feature = "bus-debugger"))]
        self.bus_debugger.borrow_mut().add_response(message, alphas);

        value
    }

    /// Builds the response from the kernel chiplet at `row`.
    fn build_kernel_chiplet_responses(
        &self,
        main_trace: &MainTrace,
        row: RowIndex,
        alphas: &[E],
        ...
        if kernel_chiplet_selector == ONE {
            ...
            #[cfg(any(test, feature = "bus-debugger"))]
            self.bus_debugger.borrow_mut().add_response(Box::new(message), alphas);
