...
fn hash_memoization_control_blocks() {
    ...
    let t_branch_id = mast_forest.ensure_node(t_branch.clone());
    ...
    let _join_node_id = mast_forest.ensure_node(join_node.clone());
...
fn hash_memoization_basic_blocks_check(basic_block: MastNode) {
    ...
    let loop_body_id = mast_forest.ensure_node(loop_body);

    let loop_block = MastNode::new_loop(loop_body_id, &mast_forest);
    let loop_block_id = mast_forest.ensure_node(loop_block.clone());

    let join2_block = MastNode::new_join(basic_block_1_id, loop_block_id, &mast_forest);
    let join2_block_id = mast_forest.ensure_node(join2_block.clone());
