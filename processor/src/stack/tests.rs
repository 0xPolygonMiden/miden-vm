...
use vm_core::{
    ...
    FieldElement, StarkField, STACK_TRACE_WIDTH, StackInputs,
};
...
fn initialize() {
    ...
    let stack = Stack::new(StackInputs::from_vec(&stack_inputs), 4, false);
...
fn shift_left() {
    ...
    let mut stack = Stack::new(StackInputs::from_vec(&inputs), 4, false);
...
fn start_restore_context() {
    ...
    let mut stack = Stack::new(StackInputs::from_vec(&stack_init), 8, false);
...
fn generate_trace() {
    let inputs = [1u64, 2, 3, 4];
    let mut stack = Stack::new(StackInputs::from_vec(&inputs), 16, false);
