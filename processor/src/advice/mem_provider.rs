...
impl AdviceProvider for MemAdviceProvider {
    ...
    fn get_merkle_path(
        ...
        let index = NodeIndex::from_elements(depth, index)
            .map_err(|_| ExecutionError::MerkleNodeIndex(*depth, *index))?;
        self.store.get_path(root, index).map_err(ExecutionError::MerkleSetLookupFailed)
    }
    ...
    fn update_merkle_node(
        ...
        let merkle_index = NodeIndex::from_elements(depth, index)
            .map_err(|_| ExecutionError::MerkleNodeIndex(*depth, *index))?;
        let new_root = self
            .store
            .set_node(root, merkle_index, value)
            .map_err(ExecutionError::MerkleSetLookupFailed)?;
        self.get_merkle_path(new_root, depth, index)
    }
