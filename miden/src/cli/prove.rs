...
pub struct ProveCmd {
    ...
    /// Hasher primitive to generate the proof; will define the security level. Possible values:
    /// [blake3-192, blake3-256, rpo-256]
    #[structopt(short = "h", long = "hasher", default_value = "blake3-192")]
    hasher: HashFunction,
    /// Path to input file
...
impl ProveCmd {
    pub fn get_proof_security(&self) -> ProofOptions {
        match self.hasher {
            HashFunction::Blake3_192 => ProofOptions::with_96_bit_security(),
            HashFunction::Blake3_256 => ProofOptions::with_128_bit_security(),
            HashFunction::Rpo256 => ProofOptions::with_rpo_128_bit_security(),
        }
    }
