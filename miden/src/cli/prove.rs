...
pub struct ProveCmd {
    ...
    /// Specifies if the RPX Hash should be used instead of RPO
    #[clap(long = "use-rpx", requires = "recursive", default_value = false)]
    use_rpx: bool,
...
impl ProveCmd {
    pub fn get_proof_options(&self) -> Result<ProvingOptions, ExecutionOptionsError> {
        ...
        Ok(match self.security.as_str() {
            "96bits" => {
                if self.recursive {
                    ProvingOptions::with_96_bit_security_recursive(self.use_rpx);
                } else {
            ...
            "128bits" => {
                if self.recursive {
                    ProvingOptions::with_128_bit_security_recursive(self.use_rpx);
                } else {
                    ProvingOptions::with_128_bit_security();
                }
            }
