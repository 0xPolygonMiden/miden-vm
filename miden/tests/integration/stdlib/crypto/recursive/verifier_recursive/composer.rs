...
use winter_air::{proof::Table, Air, DeepCompositionCoefficients, EvaluationFrame, FieldExtension};
...
impl<E: FieldElement> DeepComposer<E> {
    ...
    /// - For each H_i(x), compute H'_i(x) = (H_i(x) - H_i(z^m)) / (x - z^m), where m is the total
...
fn get_conjugate_values<E: FieldElement>(
    ...
    if extension.is_none() {
        ...
        Some((z.conjugate(), trace_state.iter().map(|v| v.conjugate()).collect()))
    }
}
