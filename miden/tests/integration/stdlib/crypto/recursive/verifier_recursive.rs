...
use vm_core::{FieldElement, QuadExtension};
...
pub fn verify(
    ...
    let _expected_seed = Rpo256::hash(&public_coin_seed);
    ...
    let _fri_remainder_size = air.options().to_fri_options().fri_remainder_size(domain_size);
    let _num_fri_layer = air.options().to_fri_options().num_fri_layers(domain_size);
    ...
    // TODO: Get the needed advice data for FRI verification
