...
fn range_check_once() {
    ...
    build_op_test!(asm_op, &stack).prove_and_verify(stack, false, vec![StdLibrary::default()]);
}
...
fn range_check_multi() {
    ...
    let stack = vec![5, 5, 5];
    build_test!(source, &stack).prove_and_verify(stack, false, vec![StdLibrary::default()]);
}
