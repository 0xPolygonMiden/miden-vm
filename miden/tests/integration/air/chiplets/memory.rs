...
fn mem_load() {
    ...
    build_op_test!(asm_op).prove_and_verify(vec![], false, vec![StdLibrary::default()]);
}
...
fn helper_mem_store() {
    ...
    let trace = build_test!(asm_op, &pub_inputs).execute(vec![StdLibrary::default()]).unwrap();
...
fn helper_write_read() {
    ...
    let trace = build_test!(source, &pub_inputs).execute(vec![StdLibrary::default()]).unwrap();
