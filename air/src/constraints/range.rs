use crate::{
    ...
    trace::range::{P1_COL_IDX, Q_COL_IDX, S0_COL_IDX, S1_COL_IDX, V_COL_IDX},
...
pub fn enforce_constraints<E: FieldElement>(frame: &EvaluationFrame<E>, result: &mut [E]) {
    ...
    // Constrain the transition between rows in the 16-bit section of the table.
    ...
    E: FieldElement<BaseField = Felt> + ExtensionOf<F>,
{
    ...
    // Enforce p1.
    enforce_running_product_p1(main_frame, aux_frame, alpha, &mut result[..]);
}
...
/// Constrain that jumps between two consecutive rows of the 16-bit table is at most 8.
fn enforce_16bit<E: FieldElement>(frame: &EvaluationFrame<E>, result: &mut [E]) -> usize {
    let constraint_count = 1;

    // Values in column v can "jump" at most 8 between two consecutive rows.
    result[0] = frame.change(V_COL_IDX)
        * (frame.change(V_COL_IDX) - E::ONE)
        * (frame.change(V_COL_IDX) - E::from(2_u8))
        * (frame.change(V_COL_IDX) - E::from(3_u8))
        * (frame.change(V_COL_IDX) - E::from(4_u8))
        * (frame.change(V_COL_IDX) - E::from(5_u8))
        * (frame.change(V_COL_IDX) - E::from(6_u8))
        * (frame.change(V_COL_IDX) - E::from(7_u8))
        * (frame.change(V_COL_IDX) - E::from(8_u8));
