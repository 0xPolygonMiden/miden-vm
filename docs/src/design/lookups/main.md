...
Zero knowledge virtual machines frequently make use of lookup arguments to enable performance optimizations. Miden VM uses two types of arguments: multiset checks and a multivariate lookup based on logarithmic derivatives known as LogUp. A brief introduction to multiset checks can be found [here](https://0xpolygonmiden.github.io/miden-vm/design/lookups/multiset.html). The description of LogUp can be found [here](https://eprint.iacr.org/2022/1530.pdf).
