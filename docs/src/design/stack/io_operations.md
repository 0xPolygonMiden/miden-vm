...
`MLOAD` operation loads the first element from the specified memory address onto the stack. The top element in the stack before the execution of this operation is the memory address which is been used to fetch a word(4 consecutive field elements) and then the *first* element of this word is pushed to the top of the stack. The diagram below illustrates this graphically.
...
$$
v_{new} = \alpha_4 \cdot s_0' + \sum_{i=0}^2\alpha_{i+5} \cdot h_i
$$

$$
v_{old} = \alpha_8 \cdot s_0' + \sum_{i=0}^2\alpha_{i+9} \cdot h_i
$$
...
> $$
b_{aux}' \cdot \left(\alpha_0 + \alpha_1 \cdot 8 + \alpha_2 \cdot s_0 + \alpha_3 \cdot clk + v_{new} + v_{old}\right) = b_{aux} \text{ | degree } = 2
$$

where $b_{aux}$ is the running product column of auxiliary table, $clk$ is the clock cycle and $\alpha_0$, $\alpha_1$, $\alpha_2$ etc... are random values sent from the verifier to the prover for use in permutation checks. We are not including context in the permutation check as its not yet been implemented. We plan to add context to the memory in $v3.0$. Values for the helper registers $h_0, h_1,  h_2$ are provided by the VM non-deterministically and represent last 3 elements of old memory at specified address. 

The $8$ in the permutation check is the unique identifier of `MEMORY` operation which has been explained [here](../stack/unique_identifier.md#identifiers).
...
$$
v_{new} = \sum_{i=0}^3\alpha_{i+4} \cdot s_i'
$$

$$
v_{old} = \sum_{i=0}^3\alpha_{i+8} \cdot s_i'
$$
...
$$
v_{new} = \alpha_4 \cdot s_0' + \sum_{i=1}^3\alpha_{i+4} \cdot h_i
$$

$$
v_{old} = \sum_{i=0}^3\alpha_{i+8} \cdot h_i
$$
...
where $b_{aux}$ is the running product column of auxiliary table, $clk$ is the clock cycle and $\alpha_0$, $\alpha_1$, $\alpha_2$ etc... are random values sent from the verifier to the prover for use in permutation checks. Values for the helper registers $h_0, h_1,  h_2, h_3$ are provided by the VM non-deterministically and represent old memory at the specified address. We are not including context in the permutation check as its not yet been implemented. We plan to add context to the memory in $v3.0$. 
...
where $b_{aux}$ is the running product column of auxiliary table, $clk$ is the clock cycle and $\alpha_0$, $\alpha_1$, $\alpha_2$ etc... are random values sent from the verifier to the prover for use in permutation checks. The value of selector flag of `memory` co-processor is $1, 1, 1$. Values for the helper registers $h_0, h_1, h_2, h_3$ are provided by the VM non-deterministically and represent old memory at the specified address. We are not including context in the permutation check as its not yet been implemented. We plan to add context to memory in $v3.0$.

The $8$ in the permutation check is the unique identifier of `MEMORY` operation which has been explained [here](../stack/unique_identifier.md#identifiers).

We stored the old memory values in the helper registers during the lookup.
