...
| adv_pipe <br> - *(2 cycles)*     | [C, B, A, a, ... ] | [F, E, D, a', ... ] | $[D, E, F] \leftarrow permute(A, adv_stack.pop(4), adv_stack.pop(4))$ <br> $a' \leftarrow a + 2$ <br> Pops the next two words from the advice stack, inserts them into memory at address $a$ sequentially. Subsequently, performs a Rescue Prime Optimized permutation using the 2 words of the advice stack as the rate portion of the hasher state, and the deepest word of the operand stack as the capacity (word A), the digest output will be in the second word of the operand stack (word E). <br> Fails if the advice stack has fewer than $8$ values. |
...
| mem_stream <br> - *(2 cycles)* | [C, B, A, a, ... ] | [F, E, D, a', ... ] | $[D, E, F] \leftarrow permute(A, mem[a], mem[a+1])$ <br> $a' \leftarrow a + 2$ <br> Read two sequential words from memory starting at address $a$. Subsequently, performs a Rescue Prime Optimized permutation using the 2 words read from memory as the rate portion of the hasher state, and the deepest word of the operand stack as the capacity (word A), the digest output will be in the second word of the operand stack (word E). |
