...
* **Advice stack**: values can be moved onto the stack from the advice stack by popping them from the advice provider. There is no limit on the number of values in the advice stack.
