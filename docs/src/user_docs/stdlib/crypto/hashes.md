...
## BLAKE3
Module `std::crypto::hashes::blake3` contains procedures for computing hashes using [BLAKE3](https://blake3.io/) hash function. The inputs and outputs elements are assumed to contain one 32-bit value per element.
...
## SHA256
Module `std::crypto::hashes::sha256` contains procedures for computing hashes using [SHA256](https://en.wikipedia.org/wiki/SHA-2) hash function. The inputs and outputs elements are assumed to contain one 32-bit value per element.
